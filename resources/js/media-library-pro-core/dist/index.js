...
function findObjectIndexByUuid(rootState, uuid) {
...
var hasOwnProperty$5 = function (_ref4) {
...
var isThenable = function isThenable(thing) {
  return thing && (isObject$1(thing) || isFunction$1(thing)) && isFunction$1(thing.then) && isFunction$1(thing["catch"]);
}; // original code
...
var platform$1 = _objectSpread(_objectSpread({}, utils$1), platform);
  ...
  _createClass(AxiosHeaders, [{
    ...
    value: function set(header, valueOrRewrite, rewrite) {
      ...
      if (utils.isPlainObject(header) || header instanceof this.constructor) {
        ...
        var _iterator = _createForOfIteratorHelper$1(header.entries()),
            _step;
...
var isURLSameOrigin = platform$1.hasStandardBrowserEnv ? // Standard browser envs have full support of the APIs needed to test
// whether the request URL is of the same origin as current location.
function standardBrowserEnv() {
  var msie = platform$1.navigator && /(msie|trident)/i.test(platform$1.navigator.userAgent);
...
var cookies = platform$1.hasStandardBrowserEnv ? // Standard browser envs support document.cookie
{
...
var headersToObject = function headersToObject(thing) {
  return thing instanceof AxiosHeaders ? _objectSpread$1({}, thing) : thing;
};
/**
...
var resolveConfig = (function (config) {
  ...
  if (utils.isFormData(data)) {
    if (platform$1.hasStandardBrowserEnv || platform$1.hasStandardBrowserWebWorkerEnv) {
      ...
      headers.setContentType([type || 'multipart/form-data'].concat(tokens).join('; '));
    }
  } // Add xsrf header
...
var supportsRequestStream = isReadableStreamSupported && test(function () {
  ...
  var hasContentType = new Request(platform$1.origin, {
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 4:
            ...
            _request = new Request(platform$1.origin, {
              method: 'POST',
              body: body
            });
            _context2.next = 8;
 ...
 */
async function vaporUpload(file, options = { signedStorageUrl: '/vapor/signed-storage-url', withCredentials: true }) {
    const response = await api.post(options.signedStorageUrl, {
...
async function uploadFile({ routePrefix, file, uuid, cancelTokenSource, vapor, vaporSignedStorageUrl, uploadDomain, withCredentials, headers, onUploadProgress, }) {
    if (vapor) {
        ...
        return api.post(uploadDomain + '/' + routePrefix + '/post-s3', {
            uuid: response.uuid,
            key: response.key,
...
function getMediaPreview(file, maxSizeForPreviewInBytes) {
...
function getFileTypeIsAllowed(fileType, accept) {
    if (!accept.length) {
        return true;
    }
...
function formatLaravelErrors(name, errors) {
    const prefix = `${name}.`;
...
class MediaLibraryClass {
    constructor(options) {
        ...
        this.translations =
            window.mediaLibraryTranslations || options.translations
                ? mergeDeep(translations.defaultTranslations, window.mediaLibraryTranslations || options.translations)
                : translations.defaultTranslations;
        window.mediaLibraryTranslations = this.translations;
        this.subscribers = [];
    }
    createMedia(attributes) {
        if (!attributes.uuid) {
        ...
        return {
            attributes: {
                ...
                original_url: null,
                ...attributes,
                custom_properties: typeof attributes.custom_properties === 'string'
    ...
    async addInvalidMedia(file, validationErrors) {
    ...
    async setClientPreview(uuid, file) {
        const client_preview = await getMediaPreview(file, this.config.maxSizeForPreviewInBytes);
        this.setProperty(uuid, 'client_preview', client_preview);
    }
    async getFileValidationErrors(file) {
        const validationErrors = [];
        if (this.config.validationRules) {
            if (!getFileTypeIsAllowed(file.type, this.config.validationRules.accept)) {
                validationErrors.push(`${this.translations.fileTypeNotAllowed} ${this.config.validationRules.accept.join(', ')}`);
            }
        ...
        try {
            ...
            if (error) {
                validationErrors.push(error.message || error);
            }
        }
        return validationErrors;
    }
    addMediaObject(newMediaObject) {
        this.changeState((state) => {
    ...
    setProperty(uuid, key, value) {
        const index = findObjectIndexByUuid(this.state, uuid);
        if (index !== -1) {
            this.changeState((state) => {
                set_1(state.media[index], key, value);
            });
        }
    }
    setCustomProperty(uuid, key, value) {
        const index = findObjectIndexByUuid(this.state, uuid);
        this.changeState((state) => {
    ...
    setOrder(uuids, sortArray = true) {
        this.changeState((state) => {
            if (sortArray) {
                state.media.sort((a, b) => uuids.indexOf(a.attributes.uuid) > uuids.indexOf(b.attributes.uuid) ? 1 : -1);
            }
    ...
    setClientValidationErrors(uuid, errors) {
        const index = findObjectIndexByUuid(this.state, uuid);
        if (index !== -1) {
            this.changeState((state) => {
    ...
    getCustomPropertyInputErrors(uuid, propertyName) {
    ...
    getNameInputErrors(uuid) {
        if (!this.state.validationErrors) {
            return [];
        }
        return this.state.validationErrors[`${uuid}.name`] || [];
    }
    upload(uuid, file, cancelTokenSource) {
        return new Promise((resolve) => {
    ...
    async replaceMedia(uuid, file) {
        ...
        this.cancelUpload(uuid);
        const index = findObjectIndexByUuid(this.state, uuid);
        const oldObject = lodash.cloneDeep(this.state.media[index]);
        if (index === -1) {
            return;
        }
        ...
        const newValidationErrors = Object.entries(this.state.validationErrors).reduce((validationErrors, [key, value]) => {
        ...
        this.changeState((state) => {
            ...
            state.media[index].attributes.name = file.name;
...
function parseTranslation(translationString, variables) {
...
function buildRuleHelpText({ validationRules = {}, maxItems, fileTypeHelpText, }) {
    ...
    let ruleHelpText = '';
    ruleHelpText = addToRuleHelpText(ruleHelpText, maxItems
    ...
    if (fileTypeRules && !fileTypeHelpText) {
        ...
        ruleHelpText = addToRuleHelpText(ruleHelpText, fileTypeRules.reduce((ruleHelpText, rule, i) => {
    ...
    if (fileSizeRules.min) {
        const minSizeString = fileSizeRules.min > 1024 ? (fileSizeRules.min / 1024).toFixed(2) + 'MB' : fileSizeRules.min + 'KB';
        ruleHelpText = addToRuleHelpText(ruleHelpText, `> ${minSizeString}`);
    }
    if (fileSizeRules.max) {
        const maxSizeString = fileSizeRules.max > 1024 ? (fileSizeRules.max / 1024).toFixed(2) + 'MB' : fileSizeRules.max + 'KB';
        ruleHelpText = addToRuleHelpText(ruleHelpText, `< ${maxSizeString}`);
    }
    return ruleHelpText;
}

export { MediaLibraryClass as MediaLibrary, buildRuleHelpText, formatLaravelErrors, getFileTypeIsAllowed, getMediaPreview, getObjPaths, isObject$2 as isObject, mergeDeep, parseTranslation, sanitizeForInput };
