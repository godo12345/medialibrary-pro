...
class MediaLibraryProServiceProvider extends ServiceProvider
{
    ...
    public function registerBladeDirectives(): self
    {
        Blade::directive('mediaLibraryScripts', function () {
            ...
            if (config('media-library.include_sortable_cdn_script', true)) {
                $scripts .= <<<'html'
        ...
        Blade::directive('mediaLibraryIcons', function () use ($icons) {
            return $icons;
        });

        Blade::directive('mediaLibraryStyles', function () use ($icons) {
            $styles = route('media-library-styles');
